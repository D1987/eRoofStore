...
import com.eroofstore.model.BillingAddress;
import com.eroofstore.model.Customer;
import com.eroofstore.model.ShippingAddress;
    ...
    public String registerCustomerPost(@Valid @ModelAttribute("customer") Customer customer, BindingResult result, Model model){
        ...
        for (int i = 0; i < customerList.size(); i++){
            if (customer.getCustomerEmail().equals(customerList.get(i).getCustomerEmail())){
                model.addAttribute("emailMsg", "Такой email уже существует");
                return "registerCustomer";
            }

            if (customer.getUsername().equals(customerList.get(i).getUsername())){
                model.addAttribute("usernameMsg", "Такой логин уже существует");
                return "registerCustomer";
            }
        }
